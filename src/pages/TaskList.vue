    ...
    methods: {
      async AgregueAMiDia(){
        ...
        if (task == null) {
          ...
          const res = await axios.put('https://stickyquickconnections.jose-albertoa97.repl.co/api/midia/'+task.id);
  ...
  async AgregueAImportante(){
    let task = JSON.parse(localStorage.getItem('selectedItem'));
        if (task == null) {
          ...
          const res = await axios.put('https://stickyquickconnections.jose-albertoa97.repl.co/api/important/'+task.id);
        this.result = res.status;
          location.reload();
          alert(res.data);
          deleteItem();
      ...
      getTodos(){
        axios.get('https://stickyquickconnections.jose-albertoa97.repl.co/api/task').then( response => {
