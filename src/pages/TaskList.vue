    ...
    methods: {
      async AgregueAMiDia(){
        ...
        if (task == null) {
          ...
          const res = await axios.put('https://BackEnd.randallcs15.repl.co/api/midia/'+task.id);
  ...
  async AgregueAImportante(){
    let task = JSON.parse(localStorage.getItem('selectedItem'));
        if (task == null) {
          ...
          const res = await axios.put('https://BackEnd.randallcs15.repl.co/api/important/'+task.id);
        this.result = res.status;
          location.reload();
          alert(res.data);
          deleteItem();
      ...
      async AgregueAPlaneado(){
        ...
        if (task == null) {
          ...
          const res = await axios.put('https://BackEnd.randallcs15.repl.co/api/planeado',{
            id:task.id,
      ...
      getTodos(){
        axios.get('https://BackEnd.randallcs15.repl.co/api/task').then( response => {
      ...
      ,
       saveNote(){
        ...
        axios.post('https://BackEnd.randallcs15.repl.co/api/note',postData)
          .then((response) => {
