    ...
    methods: {
      async AgregueAMiDia(){
        ...
        if (task == null) {
          ...
          const res = await axios.put('https://backend.derianbustos.repl.co/api/midia/'+task.id);
  ...
  async AgregueAImportante(){
    let task = JSON.parse(localStorage.getItem('selectedItem'));
        if (task == null) {
          ...
          const res = await axios.put('https://backend.derianbustos.repl.co/api/important/'+task.id);
        this.result = res.status;
          location.reload();
          alert(res.data);
          deleteItem();
      ...
      async AgregueAPlaneado(){
        ...
        if (task == null) {
          ...
          const res = await axios.put('https://backend.derianbustos.repl.co/api/planeado',{
            id:task.id,
      ...
      getTodos(){
        axios.get('https://backend.derianbustos.repl.co/api/task').then( response => {
